var board = JXG.JSXGraph.initBoard("meanValueTheorem", {
...
var g = function (x) {
  return (
    JXG.Math.Numerics.D(f)(x) - (p[1].Y() - p[0].Y()) / (p[1].X() - p[0].X())
  );
};
...
var r = board.create(
  ...
  [
    function () {
      return JXG.Math.Numerics.root(g, (p[0].X() + p[1].X()) * 0.5);
    },
    function () {
      return f(JXG.Math.Numerics.root(g, (p[0].X() + p[1].X()) * 0.5));
    },
    graph,
  ],
